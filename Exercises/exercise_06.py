...
class Scrape_Saver:
    ...
    def retrieve(self, item):
        save = self.__save_location + "/" + item

        if item in os.listdir(self.__save_location):
            with open(save, 'r') as infile:
                return infile.read()
        else:
            string = ur.urlopen(self.__base_url.format(item)).read().decode()
    ...
    def __str__(self):
        ...
        for filename in os.listdir(self.__save_location):
